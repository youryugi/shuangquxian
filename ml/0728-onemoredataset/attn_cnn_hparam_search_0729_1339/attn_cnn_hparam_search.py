...
def main():
    ...
    # ── 每个 mode 用各自选出的最优组合，在从未参与选参的 test 集上跑一次，给出无偏最终指标 ──
    test_rows = []
    for mode in args.modes:
        ...
        for seed in args.seeds:
            ...
            print(f"  [FINAL TEST] {mode} seed{seed}: P={m['bbox_P']:.4f} R={m['bbox_R']:.4f} F1={m['bbox_F1']:.4f} "
                  f"attn_iou={m['attn_band_iou']:.4f} gap={gap:.4f}", flush=True)
            test_rows.append({"mode": mode, "seed": seed, **m, "attn_gap": gap})
