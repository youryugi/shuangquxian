...
IMG_DIR   = os.path.join(_REPO_ROOT, "dataset3", "augmented_utilities")
...
# —— 注意力监督相关（调参核心）——
LAM_ATT      = [0.7,1]        # 注意力 loss 权重扫描列表（abs/soft 特有）：逐个训练对比；命令行 --lam_att 可覆盖
...
class AttnDataset(Dataset):
    def __init__(self, input_size=(640, 640), hm_stride=8, sigma=4.3,
                 img_dir=None, hyp_json=None, rect_json=None):
        # img_dir/hyp_json/rect_json 可覆盖模块级默认路径，用于指向另一个数据集（跨数据集评测）
        self.img_dir = img_dir or IMG_DIR
        hyp_json = hyp_json or HYP_JSON
        rect_json = rect_json or RECT_JSON
        ...
        names = sorted(self.hyp.keys(),
                       key=lambda n: [int(x) for x in re.findall(r'\d+', n)] or [0])
        missing = [n for n in names if not os.path.exists(os.path.join(self.img_dir, n))]
        if missing:
            print(f"[AttnDataset] 跳过 {len(missing)}/{len(names)} 个缺失图片文件的标注: "
                  f"{missing[:5]}{'...' if len(missing) > 5 else ''}")
        self.names = [n for n in names if n not in set(missing)]
    ...
    def __getitem__(self, idx):
        ...
        for o in self.hyp.get(name, []):
            if o.get("label", "") != "hyperbola":
                continue
            # 新版标注工具存的是 (x_vertex, y_vertex, slope, span, thickness)，
            # 没有 width/height 字段；这里按 normalize_obj 的反向公式换算出
            # 旧抛物线光栅化函数需要的 width/height（兼容新旧两种 schema）。
            span = o.get("span", o.get("width", 100.0))
            half_w = max(1.0, span / 2.0)
            height_raw = o.get("height", o.get("slope", 1.0) * half_w / 2.0)
            mo = {"x_vertex": o["x_vertex"] * sx, "y_vertex": o["y_vertex"] * sy,
                  "width": span * sx, "height": height_raw * sy, "thickness": o["thickness"] * sy}
            meta_objs.append(mo)
