"""
自己完结・单文件版（MobileNet 主干版，默认 MobileNetV3-Large）：
  YOLO 网格(anchor-free) 矩形框检测头 + 显式注意力监督（双曲线带 mask 监督注意力图）。
  本文件由 attn_cnn_yolo_resnet.py 改造而来，**唯一的核心区别是把 ResNet 主干换成
  torchvision 的 MobileNet（默认 mobilenet_v3_large，可 --backbone 切 mobilenet_v2 /
  mobilenet_v3_small）作为轻量特征提取网络**。
  其余（YOLO 风格检测、注意力 none/abs/soft、数据集、训练、评估、划分）全部保持不变，
  以保证与 resnet / vgg16 / from-scratch 版的公平对比（同一套 head、同一 stride4/stride8 接口）。

【MobileNet 主干如何接到原来的检测/注意力 head】
  原网络有两个关键特征点：
    - f_s4 : stride 4（160×160）→ 喂给注意力 head（ATTN_STRIDE=4 对齐 band）
    - f_s8 : stride 8（ 80× 80）→ 喂给 heatmap/wh/offset 检测 head（HM_STRIDE=8）
  MobileNet 的主干是 torchvision 的 `model.features`（一串 InvertedResidual）。
  不写死层数索引：对 features 做一次 dummy forward，按**实际输出分辨率**自动把它切成
  「stride≤4 的前半段」当 f_s4、「其后到 stride8 的一段」当 f_s8，
  从而兼容 v2 / v3-large / v3-small 以及不同 torchvision 版本。stride16/32 的深层不使用。
  通道数随变体不同（forward 内自动推断，不写死）。

【输入通道】数据集是单通道灰度图；MobileNet 第一层要 3 通道，forward 里把灰度复制成 3 通道再喂入。
【预训练】默认加载 ImageNet 预训练权重（--no-pretrained 可关闭，离线环境请关闭）。
【确定性】MobileNet 的 SE 模块含自适应池化，部分 torch 版本无确定性 backward；
  故本文件把 use_deterministic_algorithms 设为 warn_only=True（仍固定随机种子，只是遇到
  无确定性实现的算子降级为警告而非报错），避免训练中途崩溃。
...
BACKBONE       = "mobilenet_v3_large"    # 主干网络：mobilenet_v2 / mobilenet_v3_large / mobilenet_v3_small（命令行 --backbone 可覆盖）
PRETRAINED     = True                    # 是否加载 ImageNet 预训练权重（离线环境改 False / 用 --no-pretrained）
...
BASE_CH      = 32                   # （MobileNet 主干自带固定通道数，此项保留兼容，网络不再使用）
...
def set_seed(seed):
    ...
    # MobileNet 的 SE(自适应池化) 在部分 torch 版本没有确定性 backward：用 warn_only=True
    # 仍固定随机种子，只是把「无确定性实现」的算子从报错降级为警告，避免训练中途崩溃。
...
# 3. 模型（MobileNet 主干）
# ══════════════════════════════════════════════════════════════════════════════
def _split_stride_indices(feature_modules, probe=64):
    """对 torchvision backbone 的 features 序列做一次 dummy forward，
    返回 (i4, i8)：最后一个输出 stride≤4 的模块索引、最后一个输出 stride≤8 的模块索引。
    据此把 backbone 切成 stride4 段(→注意力) 与 stride8 段(→检测)，不写死层数、兼容不同变体。"""
    x = torch.zeros(1, 3, probe, probe)
    i4 = i8 = -1
    with torch.no_grad():
        for i, mdl in enumerate(feature_modules):
            x = mdl(x)
            stride = probe // x.shape[-1]
            if stride <= 4:
                i4 = i
            if stride <= 8:
                i8 = i
    if i4 < 0 or i8 <= i4:
        raise RuntimeError(f"failed to locate stride4/stride8 stages (i4={i4}, i8={i8})")
    return i4, i8


class MobileNetBackbone(nn.Module):
    """MobileNet 编码器（torchvision，默认 mobilenet_v3_large）：输出两个特征
        f_s4 : stride 4  → 给注意力 head
        f_s8 : stride 8  → 给 heatmap/wh/offset 检测 head
    通过对 model.features 做一次 dummy forward，按实际输出分辨率自动切出 stride4 / stride8 两段
    （不写死层数索引，兼容 v2 / v3-large / v3-small 及不同 torchvision 版本）。
    通道数同样 forward 内自动推断。单通道灰度输入会在 forward 内复制成 3 通道以匹配第一层。
    """
    _WEIGHTS = {
    ...
    def __init__(self, name="mobilenet_v3_large", pretrained=True):
        super().__init__()
        ...
        feats = list(m.features.children())
        i4, i8 = _split_stride_indices(feats)       # 最后一个 stride≤4 / stride≤8 的模块索引
        self.stage_s4 = nn.Sequential(*feats[:i4 + 1])       # stride4 → 注意力
        self.stage_s8 = nn.Sequential(*feats[i4 + 1:i8 + 1])  # stride8 → 检测
        # 通道数从一次 dummy forward 自动推断（兼容不同变体）
        with torch.no_grad():
...
class AttnBBoxNet(nn.Module):
    def __init__(self, in_ch=1, base_ch=32, use_attn=True, fuse="gate",
                 pretrained=True, backbone="mobilenet_v3_large"):
        super().__init__()
        self.use_attn = use_attn
        self.fuse = fuse                       # "gate"(乘法门控) 或 "concat"(通道拼接+1x1融合)
        self.backbone = MobileNetBackbone(name=backbone, pretrained=pretrained)
...
def train_model(attn_mode, full, tr, va, n_ep, work, tag, seed, augment=False, fuse="gate",
                run_val=False, pretrained=True, backbone="mobilenet_v3_large"):
    set_seed(seed)
...
def main():
    ...
    parser.add_argument("--backbone", default=BACKBONE,
                        choices=["mobilenet_v2", "mobilenet_v3_large", "mobilenet_v3_small"],
                        help="MobileNet 主干，默认 mobilenet_v3_large")
    parser.add_argument("--no-pretrained", dest="pretrained", action="store_false", default=PRETRAINED,
                        help="不加载 MobileNet 的 ImageNet 预训练权重（离线环境用）")
