...
BACKBONE       = "efficientnet_b0"       # 主干网络：efficientnet_b0 / b1 / b2 / b3（命令行 --backbone 可覆盖）
...
BASE_CH      = 32                   # （EfficientNet 主干自带固定通道数，此项保留兼容，网络不再使用）
...
def set_seed(seed):
    ...
    # EfficientNet 的 SE(自适应池化) / StochasticDepth 在部分 torch 版本没有确定性 backward：
    # 用 warn_only=True 仍固定随机种子，只是把「无确定性实现」的算子从报错降级为警告，避免训练中途崩溃。
...
class EfficientNetBackbone(nn.Module):
    """EfficientNet 编码器（torchvision，默认 efficientnet_b0）：输出两个特征
    ...
    （不写死层数索引，兼容 b0~b3 及不同 torchvision 版本）。
...
class AttnBBoxNet(nn.Module):
    def __init__(self, in_ch=1, base_ch=32, use_attn=True, fuse="gate",
                 pretrained=True, backbone="efficientnet_b0"):
        super().__init__()
        self.use_attn = use_attn
        self.fuse = fuse                       # "gate"(乘法门控) 或 "concat"(通道拼接+1x1融合)
        self.backbone = EfficientNetBackbone(name=backbone, pretrained=pretrained)
...
def train_model(attn_mode, full, tr, va, n_ep, work, tag, seed, augment=False, fuse="gate",
                run_val=False, pretrained=True, backbone="efficientnet_b0"):
    set_seed(seed)
