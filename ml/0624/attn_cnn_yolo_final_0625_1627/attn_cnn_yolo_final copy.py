...
VIS_ONLY_FIRST_SEED = True          # 只对第一个 seed 出图（省时省空间），其余 seed 跳过
...
def _title(im, t):
    ...
    cv2.putText(im, t, (5, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)


def _overlay(base_bgr, heat01, alpha):
    H, W = base_bgr.shape[:2]
    return cv2.addWeighted(base_bgr, 1.0 - alpha, _colorize(heat01, (W, H)), alpha, 0)


def grad_cam(model, img_tensor):
    """对「融合后特征」(检测 head 的输入) 做 Grad-CAM，目标分数 = objectness 峰值。
    所有模型(none/abs/soft) 用同一个探针，得到「网络真正依赖哪片区域做检测」的热图(0..1)。
    这是 none 与 abs/soft 之间唯一公平可比的「关注」——而注意力 A 因被 band 监督、不可直接和 none 比。
    ...
    try:
        ...
        with torch.enable_grad():
            obj_logit, _, _, _ = model(img_tensor.to(device))
            obj_logit.max().backward()                       # 对最自信的检测响应反传
        feat = store["feat"]
...
def _panel_row(model, img, base_bgr, contours, gt, label, alpha):
    """单个模型、单张图 → 一行 4 panel：
       [原图+GT(蓝)/预测(绿)] | [objectness] | [注意力A / none:N/A] | [Grad-CAM(公平对比列)]。"""
    with torch.no_grad():
    ...
    cam = grad_cam(model, img.unsqueeze(0))

    p1 = base_bgr.copy(); _draw_boxes(p1, gt, (255, 0, 0)); _draw_boxes(p1, boxes, (0, 255, 0))
    _title(p1, f"{label} | GT=blue pred=green")

    p2 = _overlay(base_bgr, obj, alpha); cv2.drawContours(p2, contours, -1, (0, 0, 255), 1)
    _draw_boxes(p2, boxes, (0, 255, 0)); _title(p2, "objectness (band=red)")

    if A is not None:
        p3 = _overlay(base_bgr, A, alpha); cv2.drawContours(p3, contours, -1, (0, 0, 255), 1)
        _title(p3, "attention A")
    else:
        p3 = np.full_like(base_bgr, 60); cv2.drawContours(p3, contours, -1, (0, 0, 255), 1)
        _title(p3, "attention A: N/A (none)")

    p4 = _overlay(base_bgr, cam, alpha); cv2.drawContours(p4, contours, -1, (0, 0, 255), 1)
    _title(p4, "Grad-CAM (network focus)")
...
def visualize_compare(trained, full, idxs, out_dir, alpha=VIS_ALPHA):
    ...
    for i in idxs:
        ...
        base = (img[0].numpy() * 255.0).astype(np.uint8)
        base_bgr = cv2.cvtColor(base, cv2.COLOR_GRAY2BGR)
        band_up = cv2.resize(band[0].numpy(), (W, H), interpolation=cv2.INTER_NEAREST)
        contours, _ = cv2.findContours((band_up > 0.5).astype(np.uint8),
                                       cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        gt = [hyperbola_to_bbox(o) for o in meta["objects"]]
        rows_img = [_panel_row(model, img, base_bgr, contours, gt, label, alpha)
                    for label, model in trained]
