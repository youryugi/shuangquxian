...
# —— 注意力监督相关（调参核心）——
LAM_ATT      = [0.5,1,3,5]        # 注意力 loss 权重扫描列表（abs/soft 特有）：逐个训练对比；命令行 --lam_att 可覆盖
...
def _panel_row(model, img, peak, base_bgr, contours, gt, label, alpha):
    """单个模型、单张图 → 一行 4 panel：
    ...
    _title(p4, "Grad-CAM (GT-center sum)")
...
def visualize_compare(trained, full, idxs, out_dir, alpha=VIS_ALPHA):
    ...
    H, W = input_size
    for i in idxs:
        img, _, _, _, peak, band, meta = full[i]
        ...
        gt = [hyperbola_to_bbox(o) for o in meta["objects"]]
        rows_img = [_panel_row(model, img, peak, base_bgr, contours, gt, label, alpha)
