"""
有没有可能 当前的卷积编码器有问题
潜在问题：深度不够导致感受野无法覆盖双曲线；宽度不够没法匹配不同大小的双曲线
        深度需要基于双曲线到了最后占多少格；目前几何上感受野深度不足，dan
当前数据的情况：86% 的双曲线宽 >10 格,62% 宽 >25 格。目标普遍是"巨型"的。另外目标尺寸跨度很大

        改变深度带来的问题：注意力头从哪里接入
基线(原版)
python 0624-1attn_cnn_merged_final.py --base_ch 32 --depth shallow

# 单独测"宽度"假设
python 0624-1attn_cnn_merged_final.py --base_ch 48 --depth shallow
python 0624-1attn_cnn_merged_final.py --base_ch 64 --depth shallow

# 单独测"深度/感受野"假设
python 0624-1attn_cnn_merged_final.py --base_ch 32 --depth deep

# 两者都加(上限参考)
python 0624-1attn_cnn_merged_final.py --base_ch 64 --depth deep
...
BASE_CH      = 32                   # 网络通道基数（宽度消融：32/48/64，命令行 --base_ch 可覆盖）
DEPTH        = "shallow"            # 骨干深度（感受野消融）：shallow(原版,到1/8) / deep(到1/16再回1/8)，命令行 --depth 可覆盖
...
class UpBlock(nn.Module):
    """deep 变体专用：最近邻上采样(确定性) + 与 skip 拼接 + ConvBlock。
    用于把 1/16 的瓶颈特征还原回 1/8，使检测头分辨率(stride=8)与 shallow 版完全一致。"""
    def __init__(self, in_ch, skip_ch, out_ch):
        super().__init__()
        self.reduce = nn.Conv2d(in_ch, out_ch, 1, bias=False)   # 上采样后先 1x1 压通道
        self.conv = ConvBlock(out_ch + skip_ch, out_ch)
    def forward(self, x, skip):
        x = F.interpolate(x, scale_factor=2, mode="nearest")    # nearest 有确定性实现
        x = self.reduce(x)
        return self.conv(torch.cat([x, skip], dim=1))


class AttnBBoxNet(nn.Module):
    def __init__(self, in_ch=1, base_ch=32, use_attn=True, fuse="gate", depth="shallow"):
        super().__init__()
        assert depth in ("shallow", "deep")
        self.use_attn = use_attn
        self.fuse = fuse                       # "gate"(乘法门控) 或 "concat"(通道拼接+1x1融合)
        self.depth = depth                     # "shallow"(原版,3级下采样到1/8) / "deep"(多1级到1/16再上采样回1/8,扩感受野)
        self.down1 = DownBlock(in_ch, base_ch)
        self.down2 = DownBlock(base_ch, base_ch * 2)
        self.down3 = DownBlock(base_ch * 2, base_ch * 4)
        if depth == "deep":                    # 多一级 1/8->1/16 编码，再解码回 1/8（输出 stride 仍为 8）
        ...
        mid = base_ch * 8                      # 两种 depth 下检测头输入通道一致(=base_ch*8)，故下面 head 完全不变
        if use_attn:
    ...
    def forward(self, x):
        ...
        f3, x = self.down3(x)                  # f3@1/4(给注意力头), x@1/8
        if self.depth == "deep":
            f4, x = self.down4(x)              # f4@1/8(skip), x@1/16
            x = self.bottleneck(x)             # @1/16: 感受野更大
            feat = self.up4(x, f4)             # 上采样回 @1/8
        else:
            feat = self.bottleneck(x)          # @1/8
        a_logit = None
        if self.use_attn:
...
def train_model(attn_mode, full, tr, va, n_ep, work, tag, seed, augment=False, fuse="gate", run_val=False,
    ...
    model = AttnBBoxNet(in_ch=1, base_ch=base_ch, use_attn=use_attn, fuse=fuse, depth=depth).to(device)
...
def main():
    ...
    parser.add_argument("--base_ch", type=int, default=BASE_CH,
                        help="网络通道基数（宽度消融）：32/48/64，默认取文件里的 BASE_CH")
    parser.add_argument("--depth", default=DEPTH, choices=["shallow", "deep"],
                        help="骨干深度（感受野消融）：shallow(到1/8) / deep(到1/16再回1/8)")
    ...
    print(f"[merged] n_total={n}  modes={args.modes}  seeds={args.seeds}  "
          ...
          f"base_ch={args.base_ch}  depth={args.depth}  run_val={args.val}  70/0/30", flush=True)
    ...
    for lam in args.lam_att:
        ...
        for seed in args.seeds:
            ...
            for attn_mode in args.modes:
                tag = f"seed{seed}_{attn_mode}_lam{lam}_ch{args.base_ch}_{args.depth}"
