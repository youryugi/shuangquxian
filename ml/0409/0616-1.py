...
TRAIN_FRAC    = 0.5               # 训练集比例
VAL_FRAC      = 0.25              # 验证集比例（其余为测试集 0.25）
input_size    = (640, 640)
HM_STRIDE     = 8
batch_size    = 8
num_epochs    = 100
...
BAND_RES      = 160     # Band loss 渲染分辨率（辅助损失，IoU 无量纲，无需全分辨率）
LR            = 5e-4
HM_THRESH     = 0.30
VERTEX_THRESH = 57.0
...
def compute_refinement_losses(
    ...
    band_h=None, band_w=None,
):
    """对每个 GT 峰值做注意力池化精修，返回 (精修参数 smooth-L1, Band loss)。"""
    # band loss 在低分辨率上渲染（IoU 无量纲，等比缩放结果不变），大幅省时
    if band_h is None: band_h = input_h
    if band_w is None: band_w = input_w
    ...
    for b in range(B):
        ...
        for yx in peak_yx:
            ...
            # Band loss（像素单位，作用在精修参数上；在 band_h×band_w 低分辨率上渲染）
            gt_dx = gt_offset[b, 0, yi, xi]
            gt_dy = gt_offset[b, 1, yi, xi]
            g_xv = (xi + gt_dx) / hm_w * band_w
            g_yv = (yi + gt_dy) / hm_h * band_h
            g_w  = gw * band_w
            g_h  = gh * band_h
            g_t  = gtk * band_h

            p_xv = xv_hm / hm_w * band_w
            p_yv = yv_hm / hm_h * band_h
            p_w  = refined[0] * band_w
            p_h  = refined[1] * band_h
            p_t  = refined[2] * band_h

            fn = band_dice_loss if loss_fn == 'dice' else band_iou_loss
            b_losses.append(fn(band_h, band_w, p_xv, p_yv, p_w, p_h, p_t,
...
# ── 7. 训练 / 验证 ────────────────────────────────────────────────────────────
# AMP 混合精度：bf16 加速网络前向/反向。选 bf16 而非 fp16——focal loss 有 log/clamp(1e-6)，
# fp16 会下溢，bf16 动态范围同 fp32 更稳，且无需 GradScaler。
# 注：主方法的逐 peak 精修循环是 Python 开销，AMP 对其加速有限，主要提速网络与 focal/seg。
USE_AMP   = True
...
def _compute_total_loss(model, images, gt_hm, gt_param, gt_offset, peak_mask, gt_seg):
    ...
    l_refine, l_band = compute_refinement_losses(
        ...
        band_h=BAND_RES, band_w=BAND_RES,
    )
...
def train_one_epoch(model, loader, optimizer, device, verbose=False):
    model.train()
    total = 0.0
    n_steps = len(loader)
    for step, (images, gt_hm, gt_param, gt_offset, peak_mask, gt_seg, _) in enumerate(loader, 1):
        ...
        total += loss.item()
        if verbose:
            print(f"    step {step:03d}/{n_steps}  loss={loss.item():.5f}", flush=True)
...
def print_cv_summary(seed_results):
    keys = ["global_iou", "pixel_f1", "mAP50", "instance_recall", "mean_vertex_dist"]
    col_w = 18
    header = f"{'seed':>{col_w}}" + "".join(f"{k:>{col_w}}" for k in keys)
    sep    = "-" * len(header)
    print("\n" + "=" * len(header))
    print(f"{len(SEEDS)}-Seed (train {TRAIN_FRAC:.2f}/val {VAL_FRAC:.2f}/test {1-TRAIN_FRAC-VAL_FRAC:.2f})  "
          f"sigma={HM_SIGMA}  lam={LAM}  lam_refine={LAM_REFINE}  lam_band={LAM_BAND}  lam_seg={LAM_SEG}  lr={LR}")
    print("=" * len(header))
    print(header)
    print(sep)
    for r in seed_results:
        print(f"{r['seed']:>{col_w}}" + "".join(f"{r[k]:>{col_w}.4f}" for k in keys))
    print(sep)
    means = {k: float(np.nanmean([r[k] for r in seed_results])) for k in keys}
    stds  = {k: float(np.nanstd( [r[k] for r in seed_results])) for k in keys}
...
# ── 12. 主程序 ────────────────────────────────────────────────────────────────
def make_split(n_total, seed, train_frac=TRAIN_FRAC, val_frac=VAL_FRAC):
    """按给定种子随机划分 train/val/test。"""
...
def main():
    now      = datetime.now()
    work_dir = os.path.join(os.getcwd(), f"0616-1_{now.strftime('%m%d_%H%M')}")
    ensure_dir(work_dir)
    print("Using device:", device)
    print(f"{len(SEEDS)}-Seed (train {TRAIN_FRAC:.2f}/val {VAL_FRAC:.2f}/test {1-TRAIN_FRAC-VAL_FRAC:.2f})  "
          f"sigma={HM_SIGMA}  lam={LAM}  lam_refine={LAM_REFINE}  lam_band={LAM_BAND}  "
    ...
    for seed in SEEDS:
        set_seed(seed)
        train_idx, val_idx, test_idx = make_split(n_total, seed)
        ...
        seed_dir = os.path.join(work_dir, f"seed{seed:02d}")
        ...
        for epoch in range(1, num_epochs + 1):
            tr = train_one_epoch(model, train_loader, optimizer, device, verbose=(epoch == 1))
