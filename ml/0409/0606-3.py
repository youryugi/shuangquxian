...
# Based on 0606-2.py (grid search) + data augmentation:
#   - Horizontal flip (p=0.5): flip img/heatmap/param_map/offset_map/peak_mask;
#     negate dx channel of offset_map after flip.
#   - Brightness jitter: multiply image by Uniform(0.7, 1.3), clip to [0, 1].
#   - AugWrapper applies augmentation only to the training subset.
#   - Val / Test splits receive no augmentation.
# =========================================================
...
data_sources = [
    {
        "image_dir": r"C:/Users/79152/Desktop/github/shuangquxian/biaozhumore/Utilities_more",
        "annotation_json": r"C:/Users/79152/Desktop/github/shuangquxian/biaozhumore/Utilities_more/annotations.json",
    },
]
...
hm_sigma_list  = [1.5, 2.0, 2.5]
lambda_list    = [0.5, 1.0, 2.0]
lr_list        = [5e-4, 1e-3]
thresh_list    = [0.10, 0.20, 0.30]
#用的是最好的参数list吧
hm_sigma_list  = [1.5]
lambda_list    = [1.0]
lr_list        = [5e-4]
...
def augment_sample(
    img_np:     np.ndarray,   # [H, W]        float32 ∈ [0, 1]
    hm_np:      np.ndarray,   # [hm_h, hm_w]
    pm_np:      np.ndarray,   # [3, hm_h, hm_w]
    om_np:      np.ndarray,   # [2, hm_h, hm_w]  channel 0 = dx
    pk_np:      np.ndarray,   # [hm_h, hm_w]
):
    ...
    if random.random() < 0.5:
        ...
        om_flip[0] = -om_flip[0]   # negate dx after horizontal flip
        om_np  = om_flip
        pk_np  = pk_np[:, ::-1].copy()

    # Brightness jitter: scale pixel values, keep in [0, 1]
...
class AugWrapper(Dataset):
    """Wraps a Subset and applies random augmentation at __getitem__ time."""
    ...
    def __getitem__(self, idx):
        ...
        img_np = img_t.numpy()[0]       # [H, W]
        hm_np  = hm_t.numpy()[0]       # [hm_h, hm_w]
        pm_np  = pm_t.numpy()          # [3, hm_h, hm_w]
        om_np  = om_t.numpy()          # [2, hm_h, hm_w]
        pk_np  = pk_t.numpy()[0]       # [hm_h, hm_w]
...
def compute_ap50(all_tp_flags: List[bool], all_scores: List[float], n_gt: int) -> float:
    """11-point interpolated AP at IoU=0.5."""
    if n_gt == 0:
        return 0.0
...
def evaluate(model, dataset, device, input_size, obj_thresh):
    ...
    # for mAP50: collect all predictions across the dataset
    ...
    for idx in range(len(dataset)):
        ...
        # ── instance-level mAP50 ─────────────────────────────────────────
        gt_objs   = meta["objects"]
        n_gt_instances += len(gt_objs)
        gt_matched = [False] * len(gt_objs)

        # sort predictions by confidence (high → low) for greedy matching
...
def save_test_visuals(
    model, dataset, device, input_size, obj_thresh, save_dir,
):
    """
    对 dataset 中每张图像运行推理，保存三拼图：
      左: 原图  |  中: GT（绿色）  |  右: 预测（红色）
    文件名与原图相同，保存在 save_dir 下。
    """
    ensure_dir(save_dir)

    def _color_overlay(img_u8, mask, bgr):
        """在灰度图 uint8 上叠加单色 mask，返回 BGR uint8。"""
    ...
    for idx in range(len(dataset)):
        ...
        panel_gt   = _color_overlay(img_u8, gt_mask,   (0, 200, 0))    # 绿：GT
        panel_pred = _color_overlay(img_u8, pred_mask, (0, 0, 220))    # 红：预测
...
def main():
    ...
    for run_idx, (sigma, lam, lr_val) in enumerate(training_configs, start=1):
        ...
        # 训练集套上增强包装，val/test 不增强
        aug_train_set = AugWrapper(train_set)
        ...
        for thresh in thresh_list:
            ...
            # 保存当前 thresh 下的可视化结果
            vis_dir = os.path.join(exp_dir, f"visuals_thresh{thresh:.2f}")
            save_test_visuals(model, test_set, device, input_size, thresh, vis_dir)
