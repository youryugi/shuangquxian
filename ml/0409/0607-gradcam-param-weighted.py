...
# =========================================================
# Grad-CAM（推理参数·改进版：热力图加权目标信号）
# =========================================================

class GradCAMParamWeighted:
    """
    目标信号（改进版）：
      heatmap 置信度加权的参数预测期望值
        target_ch = sum( sigmoid(hm) * param_out[ch] )
      而非单点 param_out[ch, peak_y, peak_x]。
    ...
    def __call__(self, x: torch.Tensor):
        ...
        Returns
            ...
            peak   : (peak_y, peak_x) 最置信顶点坐标（仅用于可视化标记）
        """
        hm_logit, param_out, _ = self.model(x)
        hm = torch.sigmoid(hm_logit).squeeze()   # (hm_h, hm_w)
        ...
        for ch in range(N_PARAM):
            ...
            # ---- 改进的目标信号：热力图置信度加权的参数期望 ----
            target = (hm * param_out[0, ch]).sum()
            # ----------------------------------------------------
            retain = (ch < N_PARAM - 1)
...
def run(model, dataset, out_dir):
    ...
    gcam = GradCAMParamWeighted(model)
    model.eval()
    ...
    for idx in range(len(dataset)):
        ...
        cv2.imwrite(os.path.join(out_dir, f"{stem}_wcam_param.png"), vis)
...
def main():
    ap = argparse.ArgumentParser(
        description="Grad-CAM (param head, heatmap-weighted target) for HyperbolaNet"
    )
    ap.add_argument("--ckpt",    default=DEFAULT_CKPT,
                    help="path/to/best_model.pth")
    ap.add_argument("--out_dir", default=os.path.join(
                        os.path.dirname(os.path.abspath(__file__)), "gradcam_param_weighted_out"),
                    help="output directory (default: ./gradcam_param_weighted_out)")
    args = ap.parse_args()
