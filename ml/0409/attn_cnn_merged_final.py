...
AUGMENT      = False                     # 训练集数据增强开关（水平翻转 + 亮度抖动），先关着，需要确认数据集中是否已经有了翻转。
...
def augment_sample(img, hm, wh, off, peak, band):
    """训练集增强：水平翻转（标签同步翻转，x 偏移取负）+ 亮度抖动。仅作用于训练样本。"""
    ...
    img = torch.clamp(img * factor, 0.0, 1.0)  # 亮度抖动，标签不变
...
def main():
    ...
    print(f"[merged] n_total={n}  modes={args.modes}  seeds={args.seeds}  "
          f"margin={MARGIN}  lam_att_sweep={args.lam_att}  augment={args.augment}  fuse={args.fuse}  run_val={args.val}  70/15/15", flush=True)
    ...
    for lam in args.lam_att:
        ...
        for seed in args.seeds:
            ...
            print(f"\n=== lam{lam} seed {seed}  train={len(tr)} val={len(va)} test={len(te)} ===", flush=True)
