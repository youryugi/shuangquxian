...
HYPERBOLA_FILL_COLOR = (0, 255, 0, 70)
HYPERBOLA_OUTLINE_COLOR = (0, 255, 0, 160)
...
class HyperbolaAnnotator:
    ...
    def _build_ui(self):
        ...
        row += 1
        self._make_slider(control_frame, "width", self.var_width, row, 10, 600)
        row += 1
        self._make_slider(control_frame, "height", self.var_height, row, 1, 500)
        row += 1
        ...
        hint = (
            ...
            "5. Next/Prev skips to a different group automatically\n"
            "6. Annotations are synced to all images in the same group"
        )
    ...
    def get_group_members(self, filename):
        ...
        return [
            ...
            if self.get_group_name(os.path.basename(p)) == group
        ]
    ...
    def get_group_start_indices(self):
        ...
        for i, path in enumerate(self.image_paths):
            group = self.get_group_name(os.path.basename(path))
    ...
    def load_image(self):
        ...
        # Inherit from a group member that already has annotations.
        ...
        group_pos = next(i for i, idx in enumerate(starts)
        ...
        self.info_label.config(
            text=f"Group {group_pos + 1}/{len(starts)}   "
                 f"{self.current_image_name}   "
                 f"({len(members)} images in group)   "
    ...
    def refresh_preview(self, *args):
        ...
        self.draw_hyperbola_band(
            draw,
            preview_obj,
            color=HYPERBOLA_FILL_COLOR,
            outline=HYPERBOLA_OUTLINE_COLOR,
            selected=False,
        )
    ...
    def draw_hyperbola_band(self, draw, obj, color=HYPERBOLA_FILL_COLOR, outline=HYPERBOLA_OUTLINE_COLOR, selected=False):
    ...
    def on_select_object(self, _):
        ...
        self.selected_object_index = selection[0] if selection else None
        self.refresh_both_views()
    ...
    def save_current_annotations(self):
        ...
        members = self.get_group_members(self.current_image_name)
        msg = f"Current image annotations saved: {self.current_image_name}"
        if len(members) > 1:
