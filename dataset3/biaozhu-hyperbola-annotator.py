...
def hyperbola_to_bbox(obj):
    ...
    height = (y_edge - obj["y_vertex"]) + obj["thickness"]
    return {
        "label":  obj["label"],
        "x1":     round(x1, 2),
        "y1":     round(y1, 2),
        "width":  round(obj["span"], 2),
...
class HyperbolaAnnotator:
    def __init__(self, root):
        self.root = root
        self.root.title("True-Hyperbola + Rectangle Annotation Tool")
    ...
    def _build_ui(self):
        ...
        hint = (
            ...
            "   (rectangles are derived; not reversible)\n"
            "6. Export Rectangles JSON saves bbox format\n"
            "7. Next/Prev skips same-group images\n"
            "8. Annotations sync to all group images"
        )
        ...
        if os.path.exists(self.annotations_path):
            try:
                with open(self.annotations_path, "r", encoding="utf-8") as f:
                    raw = json.load(f)
                # Normalize any old-schema objects on load.
                self.annotations = {
