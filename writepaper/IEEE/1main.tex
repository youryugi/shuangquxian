...
%为什么不直接预测双曲线：一个自然的疑问是,既然我们的标注工具已经提供了每条双曲线完整的参数化描述(顶点、宽度、高度、曲率和带状厚度),为什么不训练网络直接回归这些参数。这是一个经过深思熟虑的设计选择。第一,主流的检测任务及其数据集和评价协议(基于IoU的精确率、召回率与F1)都是围绕边界框构建的;保留边界框输出使我们能够在完全一致的协议下与主流检测器和通用注意力模块进行公平比较,并将本文所提出的几何引导注意力监督带来的增益,与改变输出表示所带来的影响干净地隔离开来。第二,丰富的几何标注并没有被丢弃,而是被光栅化为条带掩膜,作为注意力分支的显式监督信号,使几何先验去塑形网络内部的特征表示,同时保持成熟而稳定的边界框检测头不变,从而使该模块易于插入到现有检测器中。第三,直接回归诸如曲率之类的双曲线参数对标注噪声、杂波、旁瓣以及相互重叠的回波都很敏感,而且各参数尺度不一使得损失难以平衡、训练不稳定,相比之下"先检测再拟合"的分解更为鲁棒。最后,边界框仍然与现有的定位、深度估计和反演流程相兼容,使方法易于部署。
A natural question is why, given that our annotation tool already provides a
full parametric description of each hyperbola (vertex, width, height, curvature,
and band thickness), the network is not trained to regress these parameters
directly. This is a deliberate design choice. First, the mainstream detection
task, together with its datasets and evaluation protocols (IoU-based precision,
recall, and F1), is built around bounding boxes; retaining a bounding-box output
allows a fair comparison with mainstream detectors and generic attention modules
under an identical protocol, and cleanly isolates the gain brought by the
proposed geometry-guided attention supervision from any effect of changing the
output representation. Second, the rich geometric annotation is not discarded but
rasterized into a band mask and used as an explicit \emph{supervisory} signal for
the attention branch, so that the geometric prior shapes the internal feature
representation while the mature and stable bounding-box head is kept intact,
making the module easy to plug into existing detectors. Third, directly
regressing hyperbola parameters such as curvature is sensitive to annotation
noise and clutter, side lobes, and overlapping echoes, and the disparate scales
of the parameters make the loss difficult to balance and the training unstable,
whereas a detect-then-fit decomposition is more robust. Finally, bounding boxes
remain compatible with existing localization, depth-estimation, and
reconstruction pipelines, keeping the method readily deployable.
...
These alternatives span a spectrum of output granularity. At one end, classical
contrast- or edge-based extraction followed by Hough or least-squares fitting
recovers the curve explicitly, but is fragile under clutter, side lobes, and
overlapping echoes and relies on careful parameter tuning. Bounding boxes lie at
the coarse yet robust and highly standardized end. Pixel-wise segmentation masks
fall in between: they trace the echo more tightly, but demand dense annotation,
are harder to compare consistently across methods, and still do not expose the
underlying hyperbola parameters such as vertex position or curvature. Direct
parametric regression is the most geometrically informative yet the hardest to
train stably. We therefore keep the network output at the bounding-box end and
instead inject the finer band geometry as supervision, retaining the robustness
and comparability of a standard detector while still benefiting from the
geometric prior; recovering the parametric hyperbola itself is left to future
work.
