...
\begin{abstract}
In ground penetrating radar (GPR) B-scan images, subsurface targets such as pipelines and cables appear as hyperbolic reflections, and their accurate detection is a prerequisite for target localization and parameter inversion. Existing deep learning detectors typically adopt axis-aligned bounding boxes as both the annotation and output format, overlooking the strip-like, symmetric, and parametric geometry of hyperbolic echoes, while commonly used attention mechanisms rely on self-learned channel or spatial reweighting and lack explicit guidance on where to attend. To address these issues, this letter proposes a hyperbola-band attention supervision method. A dedicated annotation tool records each target as a parametric band with a few parameters, including the vertex, slope, span, and thickness, which is then rasterized into a band mask. An attention branch in the network learns to focus on hyperbola regions under the explicit supervision of this mask, and the resulting spatial prior is injected into the detection features, with the whole network trained end to end. Experiments on a public GPR dataset show that the proposed method consistently outperforms the baseline across training-data ratios, raising the F1-score from 0.68 to 0.84 at best with markedly improved training stability, and surpasses representative self-learned attention modules, demonstrating the effectiveness of explicit geometric supervision for hyperbola detection in GPR B-scans.
\end{abstract}
...
The dataset used in this study is a publicly available GPR dataset obtained from Roboflow Universe, 
...
with the remaining images used as the test set. All experiments are conducted under ten random seeds, 
...
\begin{figure}[]
    \centering
  \subfloat[Baseline detection.]{\includegraphics[width=0.48\linewidth]{baseline-result.png}}
  \hfill
  \subfloat[Baseline attention.]{\includegraphics[width=0.48\linewidth]{baseline-attention.png}}
  \\
  \subfloat[Our detection.]{\includegraphics[width=0.48\linewidth]{our-result.png}}
  \hfill
  \subfloat[Our attention.]{\includegraphics[width=0.48\linewidth]{ours-attention.png}}
...
SAR, and remote-sensing recognition. The comparison results are summarized in Table~\ref{tab:attention_comparison}.
